...
fn layer_norm(
    ...
    let mean = g.reduce_mean(input, 1);
    let centered = g.subtraction(input, mean);
...
fn compile_encoder_layer(w: &LayerWeights) -> Executable {
    ...
    g.compile(NSQualityOfService::UserInteractive)
        .expect("encoder layer compile")
}

/// Two POST-norm encoder layers fused into one ANE graph.
fn compile_fused_two_layers(
    w0: &LayerWeights,
    w1: &LayerWeights,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if median < 5.858 {
        eprintln!(
            "  >>> FASTER than CoreML by {:.1}% <<<",
            (5.858 - median) / 5.858 * 100.0
        );
    } else {
        eprintln!(
            "  Slower than CoreML by {:.1}%",
            (median - 5.858) / 5.858 * 100.0
        );
    }
    eprintln!("═══════════════════════════════════════════════════════");
