...
fn attention_body(
    ...
    let qkv = graph.inner_product(normalized, &layer_weights.qkv_weight, embedding_dim, 3 * embedding_dim);
    ...
    let projection = graph.inner_product(attention, &layer_weights.attn_proj_weight, embedding_dim, embedding_dim);
...
fn ffn_body(
    ...
    let hidden = graph.inner_product(normalized, &layer_weights.fc_weight, embedding_dim, 4 * embedding_dim);
    ...
    let projection = graph.inner_product(hidden, &layer_weights.fc_proj_weight, 4 * embedding_dim, embedding_dim);
    let projection_bias = graph.constant(
...
pub fn build_lm_head(
    ...
    // LM head is too large for a single inner_product (73MB fp16).
    // Split into 4 chunks of ~12.5K vocab each.
    let chunk = vocab_size / 4;
    let remainder = vocab_size - 3 * chunk;
    let c0 = graph.inner_product(normalized, &wte[..chunk * embedding_dim], embedding_dim, chunk);
    let c1 = graph.inner_product(normalized, &wte[chunk * embedding_dim..2 * chunk * embedding_dim], embedding_dim, chunk);
    let c2 = graph.inner_product(normalized, &wte[2 * chunk * embedding_dim..3 * chunk * embedding_dim], embedding_dim, chunk);
    let c3 = graph.inner_product(normalized, &wte[3 * chunk * embedding_dim..], embedding_dim, remainder);
    let _output = graph.concat(&[c0, c1, c2, c3], 1);
    graph.compile(NSQualityOfService::Default)
}
