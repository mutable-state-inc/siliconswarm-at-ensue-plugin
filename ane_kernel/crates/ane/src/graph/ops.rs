...
impl Graph {
    ...
    /// Constant-weight linear projection via InnerProduct (MIL `linear` op).
    pub fn inner_product(
        ...
        let out = self.alloc(Shape {
            batch: source.shape.batch,
            channels: output_channels,
            height: source.shape.height,
            width: source.shape.width,
        });
        let weight_blob = WeightBlob::from_f32(weights);
        self.ops.push(super::GraphOp {
            op: crate::Op::InnerProduct(InnerProductOp {
                name: format!("ip_{}", out.id),
                bottom: Self::tensor_name(source),
                top: Self::tensor_name(out),
                input_channels,
                output_channels,
                weights: weight_blob,
                bias: None,
